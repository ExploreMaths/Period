...
impl<'a> Lexer<'a> {
    ...
    fn lex_token(&mut self) -> Result<Option<TokenKind>, String> {
        ...
        match c {
            ...
            '{' => {
                self.advance();
                Ok(Some(TokenKind::LBrace))
            }
            '}' => {
                self.advance();
                Ok(Some(TokenKind::RBrace))
            }
            '+' => {
                self.advance();
            ...
            '%' => {
                self.advance();
                Ok(Some(TokenKind::Percent))
            }
            '<' => {
                self.advance();
                if self.peek_char() == Some('=') {
                    self.advance();
                    Ok(Some(TokenKind::LessEq))
                } else {
                    Ok(Some(TokenKind::Less))
                }
            }
            '>' => {
                self.advance();
                if self.peek_char() == Some('=') {
                    self.advance();
                    Ok(Some(TokenKind::GreaterEq))
                } else {
                    Ok(Some(TokenKind::Greater))
                }
            }
            '=' => {
                self.advance();
    ...
    fn handle_indent(&mut self, spaces: usize) -> Result<Option<TokenKind>, String> {
        ...
        if spaces > top {
            ...
            self.pending
                .pop()
                .map(|t| Ok(Some(t.kind)))
                .unwrap_or_else(|| self.lex_token())
        } else {
            Ok(None)
        }
    }
