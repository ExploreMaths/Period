...
use crate::value::{
    ClassValue, ErrorValue, FunctionValue, Integer, Value, bigint_index, range_len,
};
...
impl<'a> Vm<'a> {
        ...
        loop {
            ...
            let frame = self.frames.last_mut().ok_or_else(|| {
                Control::RuntimeError(
                    "internal error: no active call frame".to_string(),
                    Span { line: 1, col: 1 },
                )
            })?;
    ...
    fn current_frame(&self, span: &Span) -> Result<&CallFrame, Control> {
    ...
    fn current_frame_mut(&mut self, span: &Span) -> Result<&mut CallFrame, Control> {
        ...
        match op {
            Op::Constant(idx) => {
                let value = {
                    ...
                    f.function
                        ...
                        .ok_or_else(|| {
                            Control::RuntimeError(
                                "internal error: invalid constant index".to_string(),
                                span.clone(),
                            )
                        })?
                        .clone()
            ...
            Op::StoreLocal(slot) => {
                ...
                let value = self.stack.pop().ok_or_else(|| {
                    Control::RuntimeError(
                        "stack underflow in store local".to_string(),
                        span.clone(),
                    )
                })?;
            ...
            Op::GetUpvalue(slot) => {
                let value = {
                    ...
                    f.upvalues
                        ...
                        .ok_or_else(|| {
                            Control::RuntimeError(
                                "internal error: invalid upvalue".to_string(),
                                span.clone(),
                            )
                        })?
                        .borrow()
            ...
            Op::SetUpvalue(slot) => {
                let value = self.stack.pop().ok_or_else(|| {
                    Control::RuntimeError(
                        "stack underflow in set upvalue".to_string(),
                        span.clone(),
                    )
                })?;
                let upvalue = {
            ...
            Op::LoadGlobal(idx) => {
                let (_name, value) = {
                    ...
                    let name = f
                        ...
                        .ok_or_else(|| {
                            Control::RuntimeError(
                                "internal error: invalid global name index".to_string(),
                                span.clone(),
                            )
                        })?
                        .clone();
            ...
            Op::StoreGlobal(idx) => {
                ...
                let value = self.stack.pop().ok_or_else(|| {
                    Control::RuntimeError(
                        "stack underflow in store global".to_string(),
                        span.clone(),
                    )
                })?;
                self.current_frame(span)?
                    .closure
                    .borrow()
                    .set(&name, value)
            ...
            Op::DefineGlobal { name } => {
                ...
                let value = self.stack.pop().ok_or_else(|| {
                    Control::RuntimeError(
                        "stack underflow in define global".to_string(),
                        span.clone(),
                    )
                })?;
                self.current_frame(span)?
            ...
            Op::JumpIfFalse(target) => {
                let value = self.stack.pop().ok_or_else(|| {
                    Control::RuntimeError(
                        "stack underflow in jump if false".to_string(),
                        span.clone(),
                    )
                })?;
            ...
            Op::JumpIfTrue(target) => {
                let value = self.stack.pop().ok_or_else(|| {
                    Control::RuntimeError(
                        "stack underflow in jump if true".to_string(),
                        span.clone(),
                    )
                })?;
                if Interpreter::is_truthy(&value) {
            ...
            Op::Call(arg_count) => {
                ...
                for _ in 0..*arg_count {
                    args.push(self.stack.pop().ok_or_else(|| {
                        Control::RuntimeError(
                            "stack underflow in call args".to_string(),
                            span.clone(),
                        )
                    })?);
                }
            ...
            Op::Return => {
                ...
                let frame = self.frames.pop().ok_or_else(|| {
                    Control::RuntimeError(
                        "internal error: return with no frame".to_string(),
                        span.clone(),
                    )
                })?;
            ...
            Op::BuildDict(count) => {
                ...
                for _ in 0..*count {
                    let v = self.stack.pop().ok_or_else(|| {
                        Control::RuntimeError(
                            "stack underflow in dict value".to_string(),
                            span.clone(),
                        )
                    })?;
                    let k = self.stack.pop().ok_or_else(|| {
                        Control::RuntimeError(
                            "stack underflow in dict key".to_string(),
                            span.clone(),
                        )
                    })?;
                    let key = k
                        .as_key()
                        .map_err(|m| Control::RuntimeError(m, span.clone()))?;
                    map.insert(key, v);
                }
            ...
            Op::Index => {
                ...
                let object = self.stack.pop().ok_or_else(|| {
                    Control::RuntimeError(
                        "stack underflow in index object".to_string(),
                        span.clone(),
                    )
                })?;
            ...
            Op::IndexSet => {
                ...
                let object = self.stack.pop().ok_or_else(|| {
                    Control::RuntimeError(
                        "stack underflow in index set object".to_string(),
                        span.clone(),
                    )
                })?;
                let value = self.stack.pop().ok_or_else(|| {
            ...
            Op::PropertyGet(idx) => {
                let name = self
                    ...
                    .ok_or_else(|| {
                        Control::RuntimeError(
                            "internal error: invalid property name index".to_string(),
                            span.clone(),
                        )
                    })?
                    .clone();
                let object = self.stack.pop().ok_or_else(|| {
                    Control::RuntimeError(
                        "stack underflow in property get".to_string(),
                        span.clone(),
                    )
                })?;
            ...
            Op::PropertySet(idx) => {
                ...
                let object = self.stack.pop().ok_or_else(|| {
                    Control::RuntimeError(
                        "stack underflow in property set object".to_string(),
                        span.clone(),
                    )
                })?;
                let value = self.stack.pop().ok_or_else(|| {
                    Control::RuntimeError(
                        "stack underflow in property set value".to_string(),
                        span.clone(),
                    )
                })?;
            ...
            Op::New(arg_count) => {
                ...
                for _ in 0..*arg_count {
                    args.push(self.stack.pop().ok_or_else(|| {
                        Control::RuntimeError(
                            "stack underflow in new args".to_string(),
                            span.clone(),
                        )
                    })?);
                }
            ...
            Op::Tell {
                ...
                let name = self
                    ...
                    .ok_or_else(|| {
                        Control::RuntimeError(
                            "internal error: invalid method name index".to_string(),
                            span.clone(),
                        )
                    })?
                    .clone();
                ...
                for _ in 0..*arg_count {
                    args.push(self.stack.pop().ok_or_else(|| {
                        Control::RuntimeError(
                            "stack underflow in tell args".to_string(),
                            span.clone(),
                        )
                    })?);
                }
                ...
                let object = self.stack.pop().ok_or_else(|| {
                    Control::RuntimeError(
                        "stack underflow in tell object".to_string(),
                        span.clone(),
                    )
                })?;
            ...
            Op::Import(idx) => {
                let path = self
                    ...
                    .ok_or_else(|| {
                        Control::RuntimeError(
                            "internal error: invalid import path index".to_string(),
                            span.clone(),
                        )
                    })?
                    .clone();
            ...
            Op::QualifiedGet(module_idx, name_idx) => {
                let (module_name, name) = {
                    ...
                    let module_name = f
                        ...
                        .ok_or_else(|| {
                            Control::RuntimeError(
                                "internal error: invalid module name index".to_string(),
                                span.clone(),
                            )
                        })?
                        .clone();
                    let name = f
                        ...
                        .ok_or_else(|| {
                            Control::RuntimeError(
                                "internal error: invalid qualified name index".to_string(),
                                span.clone(),
                            )
                        })?
                        .clone();
                    ...
                    indices
                        ...
                        .map(|i| {
                            f.function
                                ...
                                .ok_or_else(|| {
                                    Control::RuntimeError(
                                        "internal error: invalid export name index".to_string(),
                                        span.clone(),
                                    )
                                })
                                .cloned()
            ...
            Op::Read => {
                ...
                let path_str = match path_value {
                    ...
                    _ => {
                        return Err(Control::RuntimeError(
                            "Read path must be a string".to_string(),
                            span.clone(),
                        ));
                    }
                };
                ...
                let content = std::fs::read_to_string(&resolved).map_err(|e| {
                    Control::Error(format!("Cannot read {}: {}", resolved.display(), e))
                })?;
            ...
            Op::Write => {
                ...
                let content_value = self.stack.pop().ok_or_else(|| {
                    Control::RuntimeError(
                        "stack underflow in write content".to_string(),
                        span.clone(),
                    )
                })?;
                let path_str = match path_value {
                    ...
                    _ => {
                        return Err(Control::RuntimeError(
                            "Write path must be a string".to_string(),
                            span.clone(),
                        ));
                    }
                };
                ...
                let resolved = self.interpreter.resolve_path(&path_str);
                std::fs::write(&resolved, content_str).map_err(|e| {
                    Control::Error(format!("Cannot write {}: {}", resolved.display(), e))
                })?;
            }
            Op::BuildClass {
                name,
                ...
                let name_str = f
                    ...
                    .ok_or_else(|| {
                        Control::RuntimeError(
                            "internal error: invalid class name index".to_string(),
                            span.clone(),
                        )
                    })?
                    .clone();
                ...
                let field_names: Vec<String> = fields
                    ...
                    .map(|i| {
                        f.function
                            ...
                            .ok_or_else(|| {
                                Control::RuntimeError(
                                    "internal error: invalid field name index".to_string(),
                                    span.clone(),
                                )
                            })
                            .cloned()
                    ...
                    .collect::<Result<_, _>>()?;
                let field_init: Vec<Option<usize>> = field_init
                    .iter()
                    .map(|&i| if i == usize::MAX { None } else { Some(i) })
                    .collect();
                ...
                for method_name in method_names.iter().rev() {
                    let func = self.stack.pop().ok_or_else(|| {
                        Control::RuntimeError(
                            "stack underflow in build class method".to_string(),
                            span.clone(),
                        )
                    })?;
                ...
                let init_value = if init.is_some() {
                    Some(Box::new(self.stack.pop().ok_or_else(|| {
                        Control::RuntimeError(
                            "stack underflow in build class init".to_string(),
                            span.clone(),
                        )
                    })?))
                } else {
                ...
                match self.locals.get_mut(slots_start + *slot) {
                    Some(Value::Box(rc)) => {
                        ...
                        match &mut *cell {
                            ...
                            _ => {
                                return Err(Control::RuntimeError(
                                    "Increment requires a number".to_string(),
                                    span.clone(),
                                ));
                            }
                        }
            ...
            Op::AddLocals(target, source) => {
                ...
                if *target == *source {
                    let target_ref =
                        self.locals.get_mut(slots_start + *target).ok_or_else(|| {
                            Control::RuntimeError(
                                "internal error: invalid local slot".to_string(),
                                span.clone(),
                            )
                    ...
                    match &mut *target_ref {
                        Value::Box(rc) => {
                            ...
                            match &mut *cell {
                                ...
                                _ => {
                                    return Err(Control::RuntimeError(
                                        "Addition requires numbers".to_string(),
                                        span.clone(),
                                    ));
                                }
                            }
            ...
            Op::AppendLocalString { slot, string_idx } => {
                let s = {
                    ...
                    f.function
                        ...
                        .ok_or_else(|| {
                            Control::RuntimeError(
                                "internal error: invalid string index".to_string(),
                                span.clone(),
                            )
                        })?
                        .clone()
                        ...
                        match &mut *cell {
                            ...
                            other => {
                                let result = Self::add_values(
                                    std::mem::replace(other, Value::Nothing),
                                    Value::String(s),
                                    span,
                                )?;
                                *other = result;
            ...
            Op::AppendLocalList { slot } => {
                let item = self.stack.pop().ok_or_else(|| {
                    Control::RuntimeError(
                        "stack underflow in append local list".to_string(),
                        span.clone(),
                    )
                })?;
                let slots_start = self.current_frame(span)?.slots_start;
    ...
    fn handle_runtime_error(&mut self, msg: String, span: Span) -> Result<(), Control> {
        ...
        let target = self
            .try_stack
            .iter()
            .rposition(|f| f.frame_depth <= current_depth);
    ...
    pub(crate) fn call_value(
        ...
        match callee {
            ...
            Value::Function(fv) => {
                ...
                self.locals
                    .resize(slots_start + fv.func.local_count, Value::Nothing);
        ...
        match op {
            ...
            BinOp::Mul => match (&left, &right) {
                ...
                (Value::String(s), Value::Integer(n)) | (Value::Integer(n), Value::String(s)) => {
                    ...
                    let count = n.to_bigint().to_usize().ok_or_else(|| {
                        Control::RuntimeError(
                            "Cannot repeat string: count is too large".to_string(),
                            span.clone(),
                        )
                    })?;
    ...
    fn as_index(&self, value: &Value, len: usize, span: &Span) -> Result<usize, Control> {
        ...
        let i = if n < BigInt::from(0) {
            let neg = (-n).to_usize().ok_or_else(|| {
                Control::RuntimeError("Index out of range".to_string(), span.clone())
            })?;
            if neg > len {
                return Err(Control::RuntimeError(
    ...
    fn index_get(&self, obj: Value, idx: Value, span: &Span) -> Result<Value, Control> {
        match obj {
            ...
            Value::Range { start, stop, step } => {
                ...
                let i =
                    bigint_index(&idx, &len).map_err(|m| Control::RuntimeError(m, span.clone()))?;
    ...
    fn property_get(&self, obj: Value, name: &str, span: &Span) -> Result<Value, Control> {
        match obj {
            Value::Instance {
                ...
                match class.as_ref() {
                    Value::Class(cv) => {
                        if cv.methods.contains_key(name) {
                            return Err(Control::RuntimeError(
                                format!(
                                    "method '{}' must be called with 'tell <object> to {}'",
                                    name, name
                                ),
                                span.clone(),
                            ));
                ...
                Err(Control::RuntimeError(
                    format!("'{}' has no property '{}'", obj.type_name(), name),
                    span.clone(),
                ))
            }
            Value::Error(ev) => match name {
                ...
                _ => Err(Control::RuntimeError(
                    format!("error has no property '{}'", name),
                    span.clone(),
                )),
            },
            Value::Module(mv) => mv.env.borrow().get(name).ok_or_else(|| {
                Control::RuntimeError(format!("'{}' not found in module", name), span.clone())
            }),
            _ => Err(Control::RuntimeError(
                format!("Cannot get property '{}' from {}", name, obj.type_name()),
                span.clone(),
            )),
        }
    }
        ...
        match obj {
            Value::Instance {
                ...
                if let Some(fields) = fields {
                    ...
                    Err(Control::RuntimeError(
                        format!("Cannot set property '{}' on {}", name, obj.type_name()),
                        span.clone(),
                    ))
                }
            }
        ...
        match cls {
            Value::Class(ref cv) => {
                ...
                if simple_init {
                    ...
                    return Err(Control::RuntimeError(
                        "Class takes no init arguments".to_string(),
                        span.clone(),
                    ));
                }
                Ok(instance)
            ...
            _ => Err(Control::RuntimeError(
                format!("Cannot create instance of {}", cls.type_name()),
                span.clone(),
            )),
        }
    }
    ...
    fn call_method(
        ...
        let class = match &obj {
            ...
            _ => {
                return Err(Control::RuntimeError(
                    format!("Cannot send message to {}", obj.type_name()),
                    span.clone(),
                ));
            }
        };
        ...
        if let Some(method) = method {
            ...
            Err(Control::RuntimeError(
                format!("Cannot send message '{}' to {}", name, obj.type_name()),
                span.clone(),
            ))
        }
    }
